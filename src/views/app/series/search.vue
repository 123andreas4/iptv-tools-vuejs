<template>
  <erd-row class="m-0 p-0">
    ...
    <erd-col xl="12" v-if="!viewer">
      <label class="d-block" for="search-series">{{ $t("menu.search") }}</label>
      <erd-input class="mt-1 mb-2 w-100" id="search-series" :placeholder="$t('menu.search')" v-model="serieSearch"></erd-input>
    </erd-col>
...
export default {
  ...
  methods: {
    ...
    loadSeries(search) {
      ...
      this.isLoading = true;
      this.isLoading = true;
      httpService.get(`serie/search/${this.movieSeriePlaylist}/${encodeURI(search)}`)
      .then(res => {
        if (res.status === true) {
          let series = [...res.data];
          series.sort((a, b) => {
