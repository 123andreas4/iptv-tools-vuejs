...
export default {
  ...
  computed: {
    ...mapGetters(["xdProInstances", "settings", "tmdbGenres", "tmdbFilteredGenres", "currentUser", "movieSeriePlaylist"]),
    canDownload() {
    ...
    filteredByGenre() {
      ...
      return this.topRatedMovies.filter((movie) => {
        return movie.tmdb && movie.tmdb.genres
          ? movie.tmdb.genres.findIndex((genre) => {
    ...
    filteredMovies() {
      if (this.search === null) {
        ...
        return this.filteredByGenre.filter((movie) => {
          return (
  ...
  methods: {
    ...
    loadMovies() {
      ...
      this.isLoading = true;
      httpService.get(`movies/total/${this.movieSeriePlaylist}`)
        .then(res => {
          if (res.status === true) {
            let requests = [];
            for (var i = 0; i < res.data; i += 100) {
              requests.push(
                httpService.get(
                  `movies/browse/${this.movieSeriePlaylist}/${i}/100`
                )
              );
            }
