  ...
  <erd-row class="m-0 p-0" id="editor">
    ...
    >
      <div class="px-2">
        ...
        <label class="w-100">{{ $t("m3u.group-title") }}</label>
        <erd-input
          class="mt-1 mb-2 w-100"
          v-model="groupEditor.group_name"
        ></erd-input>
        <label class="w-100">{{ $t("m3u.parent-code") }}</label>
        <erd-input
          class="mt-1 mb-2 w-100"
    ...
    >
      <div class="px-2">
        <template v-if="activeGroupType === 1">
          <label class="w-100">{{ $t("m3u.tvg-name") }}</label>
          <erd-input
            class="mt-1 mb-2 w-100"
            v-model="streamEditor.stream_tvg_name"
          ></erd-input>
          <label class="w-100">{{ $t("m3u.tvg-id") }}</label>
          <erd-input
            class="mt-1 mb-2 w-100"
            v-model="streamEditor.stream_tvg_id"
          ></erd-input>
          <label class="w-100">{{ $t("m3u.tvg-logo") }}</label>
          <erd-input
            class="mt-1 mb-2 w-100"
            v-model="streamEditor.stream_tvg_logo"
          ></erd-input>
          <label class="w-100">{{ $t("m3u.tvg-chno") }}</label>
          <erd-input
            class="mt-1 mb-2 w-100"
            v-model="streamEditor.stream_tvg_chno"
          ></erd-input>
          <label class="w-100">{{ $t("m3u.tvg-shift") }}</label>
          <erd-input
            class="mt-1 mb-2 w-100"
            v-model="streamEditor.stream_tvg_shift"
          ></erd-input>
          <label class="w-100">{{ $t("m3u.parent-code") }}</label>
          <erd-input
            class="mt-1 mb-2 w-100"
          ...
          ></erd-input>
          <label class="w-100">{{ $t("m3u.audio-track") }}</label>
          <erd-input
            class="mt-1 mb-2 w-100"
            v-model="streamEditor.stream_audio_track"
          ></erd-input>
          <label class="w-100">{{ $t("m3u.aspect-ratio") }}</label>
          <erd-input
            class="mt-1 mb-2 w-100"
            v-model="streamEditor.stream_aspect_ratio"
          ></erd-input>
    ...
    -->
    <erd-modal
      v-show="assignSIPTV.modal"
      @close="assignSiptvModal(false)"
      :title="$t('editor.assign-siptv')"
      class="select-none"
      small
    >
      <div class="px-2 assign-logo">
        <erd-spinner
          v-if="assignSIPTV.isLoading"
          size="lg"
          overlay
          absolute
        ></erd-spinner>
        ...
        <erd-select
          ...
          id="assign-tv-guide-country"
          class="w-100 mt-1 mb-2"
          :items="assignSIPTV.countries"
          v-model="assignSIPTV.country"
        ></erd-select>
        <erd-radiobutton
          class="mt-1"
          :value="1"
          v-model="assignSIPTV.assignAll"
          >{{ $t("editor.assign-all-streams") }}</erd-radiobutton
        >
        <erd-radiobutton :value="0" v-model="assignSIPTV.assignAll">{{
          $t("editor.assign-epg-missing-only")
        }}</erd-radiobutton>
      </div>
      <template v-slot:footer>
        <erd-button
          @click="assignSiptvModal(false)"
          icon="la-times"
          class="mr-1"
          variant="danger"
          >{{ $t("general.cancel") }}</erd-button
        ...
        <erd-button
          icon="la-check"
          variant="success"
          @click="assignSiptvModal(false, true)"
          :disabled="assignSIPTV.isLoading || assignSIPTV.country.length === 0"
          >{{ $t("general.ok") }}</erd-button
        >
      </template>
    </erd-modal>
    ...
    -->
    <erd-modal
      v-show="findSiptv.modal"
      @close="findSiptvModal(false)"
      :title="`${$t('editor.find-siptv')}: ${findSiptv.title}`"
      class="select-none"
      small
    >
      <div class="px-2 find-channel-id">
        <erd-spinner
          v-if="findSiptv.isLoading"
          size="lg"
          overlay
          absolute
        ></erd-spinner>
        ...
        <erd-select
          ...
          id="logo-country"
          class="w-100 mt-1 mb-2"
          :items="findSiptv.countries"
          v-model="findSiptv.country"
        ></erd-select>
        <label class="w-100" for="find-siptv-search">{{
          $t("editor.find-logo-search")
        }}</label>
        <erd-input
          id="find-siptv-search"
          class="mt-1 mb-2 w-100"
          v-model="findSiptv.search"
          autocomplete="off"
          :placeholder="$t('menu.search')"
        ></erd-input>
        <label class="w-100" for="channels">{{ $t("editor.channel") }}</label>
        <erd-select
          ...
          id="channels"
          class="w-100 mt-1 mb-2"
          :items="filteredSiptvIDS"
          v-model="findSiptv.channel"
        ></erd-select>
      </div>
      <template v-slot:footer>
        <erd-button
          @click="findSiptvModal(false)"
          icon="la-times"
          class="mr-1"
          variant="danger"
          >{{ $t("general.cancel") }}</erd-button
        ...
        <erd-button
          icon="la-check"
          variant="success"
          @click="findSiptvModal(false, true)"
          :disabled="findSiptv.isLoading || findSiptv.channels.length === 0"
          >{{ $t("general.ok") }}</erd-button
        >
      </template>
    </erd-modal>
      ...
      <div class="pt-3 select-none" v-if="playlistSelected">
        ...
        >
          <ul class="list-unstyled mb-0">
            ...
            <li class="nav-item">
              <a href="#" @click.prevent="assignSiptvModal(true)"
                ><i class="las la-th-list mr-1"></i
                >{{ $t("editor.assign-siptv") }}</a
              >
            </li>
          </ul>
        </erd-collapse>
